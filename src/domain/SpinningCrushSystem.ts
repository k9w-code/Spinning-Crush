...
export class バトル更新マネージャー {
  ...
  public update(プレイヤー入力: "A" | "D" | "F" | null, _deltaTime: number = 16.67) {
    ...
    if (dist > 0) {
      ...
      if (this.エネミーAI難易度 === 1) {
        // レベル1：マイルド接近（少し遅めだが立ち止まらずに接近）
        // 直進速度を引き上げ、時計回り旋回も少し混ぜることでマシな動きにする
        this.エネミー速度X += (-dirX * 0.045 + eTanX * 0.04) * エネミー移動速度;
        this.エネミー速度Y += (-dirY * 0.045 + eTanY * 0.04) * エネミー移動速度;
      } 
  ...
  public get最終回避率(防御側スピード: number, 攻撃側スピード: number, ソール属性相性補正: number): number {
    ...
    const 率 = 基礎回避率 + スピード差分補正 + ソール属性相性補正;
    return Math.min(100, Math.max(0, 率)); // 0%〜100%に丸める
  }

  public get最終カウンター成功率(防御側アタック: number, 攻撃側アタック: number, ブレード属性属性補正: number): number {
    const 基礎カウンター率 = 25;
    const アタック差分補正 = (防御側アタック - 攻撃側アタック) / 40;
    const 率 = 基礎カウンター率 + アタック差分補正 + ブレード属性属性補正;
    return Math.min(100, Math.max(0, 率)); // 0%〜100%に丸める
  }

  // 反動クラッシュ (フェーズ終了時に弾き飛ばしてリセット)
  public 実行反動クラッシュ() {
    // 攻撃ゲージ強制リセットを撤廃し、選択コマンドに応じた消費コスト（30 / 50 / 70）の残量を維持するように修正
...
export function 報酬ドロップ処理(
  ...
  const 部位カテゴリコード = Math.abs(Math.floor(Number(現在のドロップカウンタ) || 0)) % 3;
  // パーツマスタ上の種別コード: "1": ブレード, "2": ウェイト, "3": ソール
  const ターゲット種別 = (部位カテゴリコード + 1).toString();

  // ② ドロップランクの決定 (50%の確率で対戦相手のステージランク、残り50%で過去のランクから均等抽選)
  let chosenRank = 敵ステージランク;
  if (敵ステージランク > 1 && Math.random() >= 0.5) {
    chosenRank = Math.floor(Math.random() * (敵ステージランク - 1)) + 1;
  }

  // ターゲット種別かつ「chosenRank」のパーツプールを抽出 (該当ランクがなければ敵ステージランク以下の全体からフォールバック)
  let 対象パーツプール = パーツマスタ.filter(p => p.種別 === ターゲット種別 && parseInt(p.ランク) === chosenRank);
  if (対象パーツプール.length === 0) {
    対象パーツプール = パーツマスタ.filter(p => {
      const r = parseInt(p.ランク);
      return p.種別 === ターゲット種別 && !isNaN(r) && r <= 敵ステージランク;
    });
  }

  if (対象パーツプール.length === 0) {
    // 例外でクラッシュさせず、全パーツから安全にフォールバック、またはGP付与
    対象パーツプール = パーツマスタ.filter(p => p.種別 === ターゲット種別);
    if (対象パーツプール.length === 0) {
      return {
        更新ドロップカウンタ: (現在のドロップカウンタ + 1) % 3,
        更新所持JP: 現在の所持JP + 50,
        更新所持GP: 現在の所持JP + 50,
        更新インベントリ: [...所持インベントリ],
        獲得パーツID: "",
        案内テキスト: "参加賞として50GPを獲得しました！",
        新規獲得フラグ: false
      };
    }
  }
  ...
  if (!既所持) {
    ...
    if (セリフ && セリフ.テキスト内容) {
      ...
      案内テキスト = "既に所持しているパーツのため、10GPにリサイクルしました。";
    }
  }
...
export class ステージ進行マネージャー {
  ...
  constructor(エネミーマスタ: エネミーマスタ行[], ステージID: string = "st001") {
    // 登場ステージID == 指定されたステージID のレコードを抽出し、並び順の昇順でソート
    this.エネミーリスト = エネミーマスタ
      .filter(e => e.登場ステージID === ステージID)
